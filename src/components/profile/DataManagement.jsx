...
const DataManagement = () => {
  ...
  const exportAllData = () => {
    try {
      ...
      const trainingHistoryData = JSON.parse(
        localStorage.getItem('bounceup_training_history') || '[]'
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 状态消息 */}
      {importStatus && (
        <div className="bg-green-100 text-green-700 p-3 rounded-md">{importStatus}</div>
      )}

      {/* 错误消息 */}
      {importError && <div className="bg-red-100 text-red-700 p-3 rounded-md">{importError}</div>}
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm">
            ...
            <p className="text-gray-700 mb-4">导入将覆盖您当前的训练记录和进度数据。是否继续？</p>

            {importData && (
              <div className="bg-gray-50 p-3 rounded-md mb-4 text-sm">
                <p>
                  <strong>用户名:</strong> {importData.user.name || '未设置'}
                </p>
                <p>
                  <strong>星星总数:</strong> {importData.user.totalStars || 0}
                </p>
                <p>
                  <strong>训练记录数:</strong> {importData.trainingHistory?.length || 0}
                </p>
                <p>
                  <strong>导出日期:</strong> {new Date(importData.exportDate).toLocaleString()}
                </p>
              </div>
            )}
